...
$isDividendQuestion = $normalizedQuestion -match '\bdividendes?\b'

$isDividendAnalyticsQuestion = (
    $isDividendQuestion -and
    $normalizedQuestion -match 'analys|rendement sur cout|yield on cost|\byoc\b|capital recupere|\bdgr\b|croissance'
)

$isPortfolioQuestion = $normalizedQuestion -match '\bportefeuilles?\b|\bpea\b|\bcto\b|compte.?titres'
$isRealEstateQuestion = $normalizedQuestion -match '\bimmobilier\b|\bbiens?\b|\bloyers?\b|\bbailleurs?\b|\blocataires?\b|\bquittances?\b'
$isTaskQuestion = $normalizedQuestion -match '\btaches?\b|\borchestrateur\b'
$isProjectQuestion = $normalizedQuestion -match '\bprojets?\b|\bkanban\b|\btrello\b|\bjira\b'
$isHabitQuestion = $normalizedQuestion -match '\bhabitudes?\b|\broutines?\b'
$isFocusQuestion = $normalizedQuestion -match '\bfocus\b|\bpomodoro\b|\bconcentration\b|\bdeep work\b'

$hasSpecificTopic = (
    $isDividendQuestion -or
    $isPortfolioQuestion -or
    $isRealEstateQuestion -or
    $isTaskQuestion -or
    $isProjectQuestion -or
    $isHabitQuestion -or
    $isFocusQuestion
)

$isGeneralQuestion = (
    $normalizedQuestion -match 'que (peut|peux|puis).*(faire|proposer)' -or
    $normalizedQuestion -match 'a quoi (sert|ca sert|cela sert)' -or
    $normalizedQuestion -match 'fonctionnalites?' -or
    $normalizedQuestion -match 'presente.*scalrise' -or
    $normalizedQuestion -match 'qu.?est.?ce que scalrise' -or
    $normalizedQuestion -match 'c.?est quoi scalrise'
) -and -not $hasSpecificTopic
...
foreach ($result in $searchResults) {
    ...
    if (
        $isDividendQuestion -and
        -not $isDividendAnalyticsQuestion -and
        $result.source -eq 'knowledge/bourse/dividendes.md'
    ) {
        $boost += 0.25
    }

    if (
        $isDividendAnalyticsQuestion -and
        $result.source -eq 'knowledge/bourse/analytique-dividendes.md'
    ) {
        $boost += 0.35
    }

    if (
        $isPortfolioQuestion -and
        (
            $result.source -eq 'knowledge/bourse/portefeuilles.md' -or
            $result.source -eq 'knowledge/bourse/portefeuiles.md'
        )
    ) {
        $boost += 0.25
    }
    ...
    if (
        $isGeneralQuestion -and
        $result.source -eq 'knowledge/general/presentation-scalrise.md'
    ) {
        $boost += 0.20
    }
...
Write-Host ""
Write-Host "Meilleurs résultats :"

$sortedResults |
    Select-Object -First 5 |
    ForEach-Object {
        Write-Host "- $($_.source) : $([Math]::Round($_.score, 4))"
    }
